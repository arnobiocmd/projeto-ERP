...
class MovimentoDao extends Model{
    ...
    public function lista($limit){
        $sql = "SELECT m.*, t.*, p.produto from movimento m, tipo_movimento t, produto p
         WHERE m.id_produto = p.id_produto AND m.id_tipo_movimento = t.id_tipo_movimento";
         if($limit){
             $sql .= "LIMIT $limit";
         }
    ...
    public function filtro($filtro){
        ...
        if($filtro->data1){
            if($filtro->data2){
                ...
                $sql .= " AND data_movimeto = '$filtro->data1'";
            }
        }
