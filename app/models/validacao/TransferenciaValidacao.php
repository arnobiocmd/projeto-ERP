...
class TransferenciaValidacao
{
    public static function salvar($transferencia)
    {
        ...
        if($transferencia->id_origem == $transferencia->id_destino){
            $validacao->getData("id_origem")->isUnico(1, "Os campos origem e destino precisam ser diferentes!");
        }
        ...
        if($produto->estoque < $transferencia->qtde_transferencia){
            $validacao->getData("id_origem")->isUnico(1, "A origem não possui estoque suficiente para está transação!");
        }
